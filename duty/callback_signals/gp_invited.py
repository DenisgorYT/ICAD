...
def groupbot(event: Event):
    ...
    msg_id = event.api.msg_op(
        1, peer_id, '👀 Обнаружена группа Ириса, пытаюсь выдать админку... (Ну наверное пытаюсь)'
    )
    try:
        if event.api('messages.setMemberRole', peer_id=peer_id,
                     member_id=group_id, role='admin') == 1:
            event.api.msg_op(
                2, peer_id, '✅ Ириска назначен администратором беседы (Не проверяй точно назначен)', msg_id
            )
